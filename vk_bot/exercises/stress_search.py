...
class StressSearchExercise:
    ...
    def _analyzed_finish_hint(self, analyzed):
        ...
        if analyzed >= MIN_ANALYZED_TO_FINISH_EARLY:
            return (
                ...
                f"можно закончить прямо сейчас, остальное разбирать не обязательно (отправить "
                f"наблюдателю на проверку — потом, отдельно, через «📨 Проверка» в меню)\n"
            )
    ...
    def _get_variants_example(self):
        """Пример на экран подтверждения Вопроса 1 — показывает, что для
        одного пункта может быть НЕСКОЛЬКО разных противоположностей/дорог,
        а не только одна. Помогает понять формат «Ещё есть дороги?»."""
        return (
            "📌 Пример: «Надо мной шутят» — 10/10 → это не одна дорога, а сразу несколько:\n"
            "· надо мной не шутят\n"
    ...
    def _send_variant_percent_question(self, user_id, item_text, item_rate, index, total, current_answer):
        ...
        (у него единственного среди перечисленных ещё нет процента). Явно
        называем текст этого варианта в вопросе — иначе не понятно, к
        какому пункту из списка относится число, которое сейчас пишут."""
        ...
        self.send_message(
            ...
            f"💭 Это твои убеждения о том, каким должен быть мир. Мир не такой, "
            f"поэтому не совпадает с твоим ожиданием — и рождается стресс, который толкает "
            f"тебя его изменить.\n\n"
            f"· Напиши число от 0 до 100 — на сколько процентов реально «{variant['text']}»\n\n"
            f"💾 «Сохранить и выйти»",
            cancel_keyboard()
        )
    ...
    def _send_variant_why_question(self, user_id, item_text, item_rate, index, total, current_answer):
        ...
        self.send_message(
            ...
            f"· Оценил «{variant['text']}» в {variant.get('percent', 0)}% реальности в твоей "
            f"жизни — на твой взгляд, именно столько процентов это происходит. Давай доберём "
            f"до 100%.\n"
            f"· Представь, как это выглядело бы при 100%. Представил? Напиши.\n\n"
            f"💾 «Сохранить и выйти»",
            cancel_keyboard()
        )
    ...
    def _get_question1_hint(self):
        return (
            ...
            "🧭 Не получается придумать противоположность? Возможно, дело не в тебе — "
            "сам пункт сформулирован неточно. Попробуй переформулировать, что именно "
            "тебе не нравится, конкретнее — и противоположность найдётся сама."
        )
    ...
    def _send_intro(self, user_id):
        self.send_message(
            ...
            "📖 Формула: Стресс = Прогноз ⚡ Реальность. Чем больше напряжение — тем ярче свет, тем "
            "виднее препятствие.\n\n"
            ...
            f"🩺 Закончить путь можно раньше 100 — кнопка «✅ Завершить и отправить» "
            f"сохранит карту (а отправить её наблюдателю на проверку — отдельный шаг "
            f"потом, через «📨 Проверка» в меню):\n"
            f"✅ Часть 1. {MIN_ITEMS_TO_FINISH_EARLY} пунктов — можно сохранить без разбора "
    ...
    def _start_analysis(self, user_id, session):
        ...
        self.send_message(
            ...
            "· Ты увидишь, где прогноз твоей карты расходится с реальностью\n\n"
    ...
    def _resume_current_question(self, user_id, session):
        ...
        if step == 1:
            ...
            if current_answer.get('ideal_details'):
                # Несколько вариантов — процент и «почему» идут парой на
                # каждый (см. handle_question), _variant_phase говорит,
                # какой из двух сейчас ждём.
                if current_answer.get('_variant_phase') == 'why':
            ...
            # Многовариантный разбор больше не задерживается на шаге 3 (см.
            # step == 2 в handle_question) — сюда доходит только
            # классический путь с одним вариантом.
            ...
            self.send_message(
                ...
                f"{self._format_answers_so_far(current_answer)}"
                f"❓ Вопрос 3/4:\n"
                f"· Оценил это в {percent}% реальности в твоей жизни — на твой взгляд, именно "
                f"столько процентов это происходит. Давай доберём до 100%.\n"
                f"· Представь, как это выглядело бы при 100%, и объясни, почему, по-твоему, "
                f"так должно быть.\n\n"
                f"💾 «Сохранить и выйти»",
                cancel_keyboard()
            )
            ...
            self.send_message(
                ...
                f"{self._format_answers_so_far(current_answer)}"
                f"❓ Вопрос 4/4:\n\n"
                f"Почему именно так должно быть, как ты написал?\n"
                f"Как правило, ответ сводится к одному:\n\n"
                f"· «Я — пуп земли и пуп вселенной.\n"
                f"· И всё должно быть по-моему».\n\n"
                f"· Это нормально так думать 😊\n\n"
                f"· Напиши свои размышления: почему так должно быть, как ты написал.\n"
                f"Если это выгодно миру — он изменится, а если не выгодно — значит, плохо "
                f"объяснил, и есть куда расти.\n\n"
                f"💾 «Сохранить и выйти»",
                cancel_keyboard()
            )
        elif step == 5:
    ...
    def handle_question(self, user_id, text, session):
        ...
        if session.get('_confirm_ideal'):
            if text_lower in CONTINUE_TEXTS:
                ...
                if variants and len(variants) > 1:
                    ...
                    current_answer['_variant_phase'] = 'percent'
                    self._save_progress(user_id, session)
                    self._send_variant_percent_question(user_id, item_text, item_rate, index, total, current_answer)
                    return
        ...
        if step == 1:
            ...
            details = current_answer.get('ideal_details')
            if details:
                # Несколько вариантов — по каждому спрашиваются ОБА
                # под-вопроса подряд (сначала процент, сразу за ним
                # «почему» — с опорой на этот же процент, пока не забылся),
                # а уже потом переходим к следующему варианту. _variant_phase
                # отличает, какой из двух сейчас ждём.
                idx = current_answer.get('_variant_idx', 0)
                phase = current_answer.get('_variant_phase', 'percent')
                ...
                if phase == 'percent':
                    ...
                    current_answer['_variant_phase'] = 'why'
                    self._save_progress(user_id, session)
                    self._send_variant_why_question(user_id, item_text, item_rate, index, total, current_answer)
                    return

                # phase == 'why' — «почему» для ТОГО ЖЕ варианта, что и
                # только что заданный процент.
                ...
                # Все варианты разобраны — считаем среднее в 'percent' (для
                # общей статистики реалистичности), склеиваем 'why', и
                # переходим к Вопросу 4/4 (он один общий на весь образ).
                ...
                current_answer.pop('_variant_phase', None)
            ...
            # Многовариантный разбор больше сюда не заходит — процент и
            # «почему» для каждого варианта разбираются подряд внутри
            # step == 2 (см. выше), к шагу 3 доходит только классический
            # путь с одним вариантом «как должно быть».
    ...
    def _finish_exercise(self, user_id, session):
        ...
        # Завершение (и раннее, и полное) само по себе НЕ отправляет
        # результат наблюдателю — это отдельный осознанный шаг через пункт
        # «📨 Проверка» в главном меню (см. handlers.show_review_menu /
        # handle_send_review). Раньше в этом самом чате уже правили похожую
        # путаницу в тексте интро ("получишь комментарий" → "появится
        # возможность отправить") — здесь та же логика: честно указываем
        # реальный следующий шаг, а не намекаем на автоотправку.
        self.send_message(
            user_id,
            f"· 🔦 Осветил {total} образов в тумане\n"
            f"· 🧠 Разобрано: {analyzed}\n"
            f"{realism_line}"
            f"{streak_text}\n\n"
            f"{self._get_separator()}\n"
            f"🔥 Топ-3:\n{top_text}\n\n"
            f"📨 Чтобы отправить наблюдателю на проверку — пункт «📨 Проверка» в меню\n\n"
            f"✨ Береги себя ❤️",
            main_menu()
        )
